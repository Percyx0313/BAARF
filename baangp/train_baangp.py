...
def save_camera_poses(args, train_dataset, outlier_ids: list = None, path="poses_outlier", name=None, pertubation_SE3=None):
    ...
    with torch.no_grad():
        ...
        print("--------------------------")
        # to use numpy, the value needs to be on cpu first.
        print("{} train rot error:   {:8.3f}".format(step, rot_error))
        print("{} train trans error: {:10.5f}".format(step, trans_error))
        print("--------------------------")
        # dump numbers

        pose_aligned_detached, gt_poses_detached = pose_aligned.detach(
        ).cpu(), gt_poses.detach().cpu()
        ...
        if pertubation_SE3 is not None:
            ...
            pertubation_pose = compose_poses(
                [pertubation_SE3, models["radiance_field"].pose_noise[outlier_ids[0]], gt_poses[outlier_ids[0]]])

            # align the pertubation pose
            center = torch.zeros(1, 1, 3, device=pertubation_pose.device)
            pertubation_centers = cam2world(
                center, pertubation_pose)[:, 0]  # [N,3]

            sim3 = edict(t0=0, t1=0, s0=1, s1=1, R=torch.eye(
                3, device=pertubation_pose.device))
            # align the camera poses
            pertubation_center_aligned = (
                pertubation_centers-sim3.t1)/sim3.s1@sim3.R.t()*sim3.s0+sim3.t0
            R_aligned = pertubation_pose[..., :3]@sim3.R.t()
            t_aligned = (-R_aligned @
                         pertubation_center_aligned[..., None])[..., 0]
...
def validate(models, train_dataset, test_dataset, step=0):
    ...
    with torch.no_grad():
        ...
        rot_error, trans_error, pose_aligned, sim3 =\
            pose_evaluate(  args,
                            models["radiance_field"].se3_refine_R.weight,
                            models["radiance_field"].se3_refine_T.weight,
                            models["radiance_field"].pose_noise,
                            gt_poses,
                            dataset='llff')
        ...
        os.makedirs(cam_dir, exist_ok=True)
        viz.plot_save_poses_llff(
            fig, pose_aligned_detached, pose_ref=gt_poses_detached, path=cam_dir, ep=step)
        # png_fname = viz.plot_save_poses_blender(fig=fig,
        #                                         pose=pose_aligned_detached,
        #                                         pose_ref=gt_poses_detached,
        #                                         path=cam_dir,
    ...
    #         # rendering
    #         rgb, opacity, depth, _,_,_,_,_ = render_image_with_occgrid(
    #             # scene
...
if __name__ == "__main__":
    ...
    # training parameters
    lr = 1.e-2 if args.dataset == 'blender' else 1e-2  # 5e-3
    lr_end = 1.e-4 if args.dataset == 'blender' else 1.e-4
    lr_pose_R = 3.e-3 if args.dataset == 'blender' else 3e-3  # 1.e-2
    lr_pose_T = 5.e-3 if args.dataset == 'blender' else 3e-3  # 1.e-2
    optim_lr_pose=1.e-4
    optim_lr_end_pose = 1.e-4
    max_steps = 40000 if args.dataset == 'blender' else 50000
    init_batch_size = 512
    ...
    #
    ema_alpha = 0.9 if args.dataset == 'blender' else 0.9
    ...
    if args.dataset == 'llff':
        ...
        aabb = torch.tensor([-aabb_scale, -aabb_scale, -aabb_scale,
                            aabb_scale, aabb_scale, aabb_scale], device=device)
        near_plane = 0
        ic(aabb_scale)
    ...
    estimator = OccGridEstimator(
        # ,contraction_type="UN_BOUNDED_SPHERE"
    ...
    pose_scheduler = torch.optim.lr_scheduler.ExponentialLR(
        pose_optimizer,
        gamma=(optim_lr_end_pose/lr_pose_R)**(1./max_steps)
    )
    schedulers["pose_scheduler"] = pose_scheduler
    optimizers["pose_optimizer"] = pose_optimizer

    lpips_net = LPIPS(net="vgg").to(device)
    def lpips_norm_fn(x): return x[None, ...].permute(0, 3, 1, 2) * 2 - 1

    def lpips_fn(x, y): return lpips_net(
        lpips_norm_fn(x), lpips_norm_fn(y)).mean()

    models, optimizers, schedulers, epoch, iteration, has_checkpoint = load_ckpt(
        save_dir=args.save_dir, models=models, optimizers=optimizers, schedulers=schedulers)
    # training
    ...
    if not has_checkpoint:
        ...
        for step in loader:
            ...
            # if render_step_size < 5e-3 and step % 2000 == 0:
            #     render_step_size /= 2
            ...
            # query rays
            rays = models["radiance_field"].query_rays(
                idx=image_ids, grid_3D=grid_3D, gt_poses=gt_poses, mode='train')
            ...
            # print(entropy)
            # +0.001*depth_loss#+entropy#+disto_loss
            alphas,betas=models["radiance_field"].depth_scale.weight[image_ids,0],models["radiance_field"].depth_scale.weight[image_ids,1]
            # depth_loss = F.smooth_l1_loss(1/(1e-6+depth.squeeze())*alphas+betas, gt_depths.squeeze())
            # ic(1/(alphas*depth+betas+1),gt_depths)
            # oppacity_reg=-(alphas_t*torch.log(alphas_t+1.e-6)+(1-alphas_t)*torch.log(1-alphas_t+1.e-6)).mean()
            # print(depth_loss)
            loss = F.smooth_l1_loss(rgb, pixels)#+entropy*0.01#+0.01*depth_loss
            # s3im loss
            ...
            for key in schedulers:
                schedulers[key].step()
            loader.set_postfix(it=step, loss="{:.4f}".format(
                scaled_train_loss.item()))
            # EMA pose
            ...
            if (args.reloc == True and (step+2500) % 5000 == 0 and step != 0 and step < max_steps-5000):
                ...
                ic("the max error camera is : ", re_localize_error_list.argmax(
                ).item(), re_localize_error_list.max().item())
            ...
            if (args.reloc == True and (step+1) % 5000 == 0 and step != 0 and step < max_steps-5000):
                ...
                save_camera_poses(args, train_dataset, outlier_ids=[
                                  outlier_id], path="poses_outlier_before")
                # init the relocalizer's parameters
                filter_ratio = args.filter_ratio
                outlier_pose_id = outlier_id
                outlier_se3_R, outlier_se3_T = models["radiance_field"].se3_refine_R.weight[outlier_pose_id].clone(
                ), models["radiance_field"].se3_refine_T.weight[outlier_pose_id].clone()
                ...
                delta_T = range_T * \
                    (torch.rand(num_hypothesis, 3).to('cuda')-0.5)
                delta_R = torch.deg2rad(
                    range_R*(torch.rand(num_hypothesis, 3).to('cuda')-0.5))
                ...
                pertubation_se3_R = torch.nn.Parameter(
                    hyp_R.clone())  # [hyp_num,3]
                pertubation_se3_T = torch.nn.Parameter(
                    hyp_T.clone())  # [hyp_num,3]
                ...
                for epoch in range(Stage_number):
                    for it in tqdm.tqdm(range(stage_iter)):
                        ...
                        hyp_pixels = hyp_pixels.unsqueeze(0).repeat(
                            train_dataset.hypothesis_cam_num, 1, 1)
                        ...
                        poses_refine = so3_t3_to_SE3(
                            pertubation_se3_R, pertubation_se3_T)  # [1, 3, 4]
                        ...
                        grid_3D = cam2world(hyp_grid_3D.squeeze().unsqueeze(
                            0), poses)  # [B, N, 3], [B, 3, 4] -> [B, 3]
                        center_3D = cam2world(
                            # [B, N, 3]
                            center_3D.squeeze().unsqueeze(0), poses)
                        ...
                        accumulated_loss = 0.9*accumulated_loss+0.1 * \
                            torch.mean(((hyp_rgb.view(-1, 3)-hyp_pixels.view(-1, 3))**2.).view(
                                train_dataset.hypothesis_cam_num, -1), dim=-1).detach()
                        F1 = F.smooth_l1_loss(
                            hyp_rgb.view(-1, 3), hyp_pixels.view(-1, 3))
                        ...
                        if it % 128 == 0:
                            ...
                            ic(outlier_pose_id)
                            save_camera_poses(args, train_dataset, outlier_ids=[
                                              outlier_pose_id], path="reloc_process", name=name, pertubation_SE3=poses_refine)
                    # filter the hypothsis pose
                    if epoch < Stage_number-1:
                        with torch.no_grad():
                            remain_pose_num = max(
                                int((filter_ratio**(epoch+1))*train_dataset.hypothesis_cam_num), 1)
                            accumulated_loss, indices = torch.topk(
                                accumulated_loss, remain_pose_num, largest=False)
                            new_pertubation_se3_R = pertubation_se3_R[indices].detach(
                            ).clone()
                            new_pertubation_se3_T = pertubation_se3_T[indices].detach(
                            ).clone()
                            # resample arount the pose
                            num_resmple_hypothesis = (
                                num_hypothesis-remain_pose_num)//remain_pose_num
                            ...
                            new_pertubation_se3_R = torch.cat(
                                [new_pertubation_se3_R, hyp_R], dim=0).view(-1, 6)
                            new_pertubation_se3_T = torch.cat(
                                [new_pertubation_se3_T, hyp_T], dim=0).view(-1, 6)
                            # ic(new_pertubation_se3.shape)
                        new_pertubation_se3_R = torch.nn.Parameter(
                            new_pertubation_se3_R).view(-1, 3)
                        pertubation_se3_R = new_pertubation_se3_R.detach().clone().requires_grad_(True)
                        new_pertubation_se3_T = torch.nn.Parameter(
                            new_pertubation_se3_T).view(-1, 3)
                        ...
                        accumulated_loss = torch.zeros(
                            train_dataset.hypothesis_cam_num).to('cuda')
                ...
                if (old_re_localize_error >= accumulated_loss.min().item()):
                    ...
                    with torch.no_grad():
                        models["radiance_field"].se3_refine_R.weight[outlier_pose_id].data.copy_(
                            pertubation_se3_R[accumulated_loss.argmin().item()].detach())
                        models["radiance_field"].se3_refine_T.weight[outlier_pose_id].data.copy_(
                            pertubation_se3_T[accumulated_loss.argmin().item()].detach())

                # visualize the hypothsis test pose
                save_camera_poses(args, train_dataset, outlier_ids=[
                                  outlier_id], path="poses_outlier_after")
            ...
            if step % 100 == 0:
                ...
                if step % 500 == 0:
                    ...
                    if step != 0:
                        validate(models, train_dataset,
                                 test_dataset, step=step)
                    # wandb.log({
                    #     "rot_error": rot_error,
            ...
            #     validate(models , train_dataset, test_dataset,step=step)
                # if step == 4000:
        save_ckpt(save_dir=args.save_dir, iteration=step, models=models,
                  optimizers=optimizers, schedulers=schedulers, final=True)
    else:
        step = iteration
    # evaluation
    ...
    with torch.no_grad():
        ...
        pose_refine = so3_t3_to_SE3(
            models["radiance_field"].se3_refine_R.weight, models["radiance_field"].se3_refine_T.weight)
        gt_poses = train_dataset.camfromworld
        if args.dataset == 'blender':
            pred_poses = compose_poses(
                [pose_refine, models["radiance_field"].pose_noise, gt_poses])
        else:

            pred_poses = so3_t3_to_SE3(
                models["radiance_field"].se3_refine_R.weight, models["radiance_field"].se3_refine_T.weight)
    ...
    for i in tqdm.tqdm(range(len(test_dataset))):
        ...
        with torch.no_grad():
            ...
            res.append(edict(psnr=psnr, ssim=ssim_val,
                       ms_ssim=ms_ssim_val, lpips=lpips_loss_val))
            # dump novel views
            ic("asdsadsadsad")
            rgb_map_cpu = rgb_map.cpu()
            gt_map_cpu = pixels.cpu()
            depth_map_cpu = invdepth_map.cpu()
            torchvision_F.to_pil_image(rgb_map_cpu).save(
                "{}/rgb_{}.png".format(test_dir, i))
            torchvision_F.to_pil_image(gt_map_cpu).save(
                "{}/rgb_GT_{}.png".format(test_dir, i))
            torchvision_F.to_pil_image(depth_map_cpu).save(
                "{}/depth_{}.png".format(test_dir, i))
    ...
    with open(quant_fname, "w") as file:
        for i, r in enumerate(res):
            file.write("{} {} {} {} {}\n".format(
                i, r.psnr, r.ssim, r.ms_ssim, r.lpips))
    ...
    os.system(
        "ffmpeg -y -framerate 30 -i {0}/rgb_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1".format(test_dir, rgb_vid_fname))
    os.system(
        "ffmpeg -y -framerate 30 -i {0}/depth_%d.png -pix_fmt yuv420p {1} >/dev/null 2>&1".format(test_dir, depth_vid_fname))
